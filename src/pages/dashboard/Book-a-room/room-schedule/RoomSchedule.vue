...
import { CalendarIcon, Captions, Clock, Copy, Link, LocateFixed, MapPin, Pencil, Trash2, User } from 'lucide-vue-next';
...
const meetingRoom = ref<MeetingRoom>();
const data = reactive<{
  date: string;
  start_time: string;
  end_time: string;
...
onMounted(async () => {
  ...
  meetingRoom.value = meetingRoomStore.meetingRooms.find((room) => room._id === id.value);
  ...
  await meetingScheduleStore.getScheduleByRoom(
    ...
    formatDate(lastDayOfWeek, false)
  );
});
...
const onEventClick = async (event: any, e: any) => {
  await meetingRoomStore.getBookingRoom(event.id as string);
...
const handleCellClick = (value: any) => {
  ...
  data.room = meetingRoom.value?._id;
  data.location = meetingRoom.value?.location;
...
<template>
  <vue-cal
    ...
    class="!h-fit border-gray-200 rounded overflow-hidden mr-2.5 vuecal--full-height-delete"
  ...
  <BookingModal @update:open="handleOpenModal" :model-value="isOpenModal" v-bind="{ ...data }" type="room" />
</template>
